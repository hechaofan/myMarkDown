...
```
npm i webpack@4.30.0 webpack-cli@3.3.1 -D //-D 是打包完就不需要了
```
...
npx webpack --config 运行对应的webpack配置文件

```
npx webpack --config webpack.config.js
或者创建webpack.custom.config.js
...
```
webpack-dev-server将打包文件放到根目录下/
index.html中修改 <script src="/bundle.js"></script>
在终端中ctrl +c 停止服务
"dev2": "webpack-dev-server --compress --hot --port 5000 --open --contentBase src",
--contentBase src 将src设置为根目录
--open 自动打开项目
--port 5000 设置端口
--hot 热更新 只将当前修改的模块打包成补丁去更新，之前是整个打包为一个文件再去更新
--compress 压缩
```
...
2. 新建`server.js`
   ...
   const config = require('./webpack.config.js');//当前目录下的webpack.config.js文件引入
   
   const app = express();
   const compiler = webpack(config); //webpack使用配置
   
   app.use(webpackDevMiddleware(compiler, {//webpack-dev-middleware打包文件放置的地方
     publicPath: '/'
   }));
...
```js
{
  ...
  exclude: /node_modules/ //babel不需要打包node_modules下的js inclide是包含
  include: path.resolve(__dirname, '../src') //babel需要打包src下的js
}
```
...
在`.babelrc`中添加插件: //json数据格式 都要加双引号
...
**ps**：（开发时更好调试，不会看到babel编译后的代码，与自己编写的代码一致）
...
3. 使用插件, 在plugins中插件对象并配置源和目标
   ...
   ```
   <img src="/assets/logo.png" alt=""> //路径为根目录下 （开发环境文件在根目录下，打包后也是在根目录下）
   ```
...
```
import $ from 'jquery' // 去找node_modules/jquery/package.json > main
```
  ...
  2. 配置loader
     ...
     module: {
       rules: [{
         ...
         use: {
           loader: 'expose-loader', //使用expose-loader将jquery注入到全局中为$
           options: '$'
         }
       }]
     }
...
- webpack.ProvidePlugin **将库自动加载到每个模块**
  ...
  2. 创建插件对象
     ...
     new webpack.ProvidePlugin({
       $: 'jquery',  //在node_modules下 找到jquery 注入到全局为$
       jQuery: 'jquery' //在node_modules下 找到jquery 注入到全局为jQuery
     })
     ```
...
1. 将开发环境和生产环境公用的配置放入base中，不同的配置各自放入prod或dev文件中（例如：mode）
   ...
   // webpack的配置文件遵循着CommonJS规范
   module.exports = merge(baseConfig,{ //合并整体暴露 merge会返还一个新的对象
     mode: 'development',
...
3. 将package.json中的脚本参数进行修改，通过`--config`手动指定特定的配置文件
   ...
   更改webpack.base.js中输出位置
   output: {
       // path.resolve() : 解析当前相对路径的绝对路径
       // path: path.resolve('./dist/'),
       // path: path.resolve(__dirname, './dist/'),
       path: path.join(__dirname, '..', './dist/'), //__dirname当前目录 ..上一级 ./dist/创建dist
       // filename: 'bundle.js',
       filename: '[name].js',
       publicPath: '/'
     },
         
   //注意相对路径和绝对路径，绝对路径带path
         path: path.join(__dirname, '..', './dist/') //这是绝对路径
       new HtmlWebpackPlugin({
         filename: 'index.html',
         template: './src/index.html',//这是相对路径
         chunks: ['index', 'other']
       }),
...
2. 创建插件对象，并定义环境变量
   ...
   new webpack.DefinePlugin({
     IS_DEV: 'true' , //会将'true' ''内的当成js处理 如果要是字符窜要用双引号
     test: '1+1' ,//test  为2 想eval函数
       test2: '"zs"' //如果要是字符窜要用双引号
   })
...
1. jsonp（淘汰）
2. cors    response headers响应头添加Access-Control-Allow-Headers: *
3. http proxy
...
需要对某个模块进行热更新时，可以通过`module.hot.accept`方法进行文件监视，**网页不会重新加载**
...
//     // import / export语法必须在顶级作用域中使用,无法在子级作用域中使用，使用common.js引入
...
- tree shaking
  ...
  tree shaking 是一个术语，通常用于打包时移除 JavaScript 中的未引用的代码(dead-code)，它依赖于 ES6 模块系统中 `import`和 `export`的**静态结构**特性。
   ...
   import / export语法必须在顶级作用域中使用,无法在子级作用域中使用，**使用common.js引入require('./math.js')  则不会移除 JavaScript 中的未引用的代码**
...
**Code Splitting**是webpack打包时用到的重要的优化特性之一，此特性能够把代码分离到不同的 bundle 中，然后可以按需加载或并行加载这些文件。代码分离可以用于获取更小的 bundle，以及控制资源加载优先级，如果使用合理，会极大影响加载时间。
...
好处：需要使用的地方才加载，比如spa应用，很多页面，首屏加载很慢，动态加载就不会在首页将所有资源加载，只有在需要的地方才会去加载，使首屏加载更快
...
// 动态倒入 import('jquery') 其实返回的就是一个promise对象
...
**SplitChunksPlugin执行顺序**

```
由上倒下再看cacheGroups中配置，是否匹配再看vendors第三方模块打包和default自己内部模块打包的权重（priority）
```
...
module.exports = {
  ...
  optimization: {
    splitChunks: {
      chunks: 'async', // 只对异步加载的模块（动态倒入）进行拆分，可选值还有all | initial
        //all 异步加载的模块（动态倒入）和静态倒入进行拆分
      ...
      cacheGroups: { // 缓存组配置,上面配置读取完成后进行拆分,如果需要把多个模块拆分到一个文件,就需要缓存,所以命名为缓存组
        vendors: { // 自定义缓存组名  vendors可以修改 用于第三方文件分离切割代码
          ...
          //name: 'vendors.js' //给打包文件命名  注意动态导入不要名字了 动态导入文件会在chunks为async和all时自动分割
        },
        default: { // 默认缓存组名 default可以修改 用于自己业务代码分离切割
...
在引入一些第三方模块时，例如vue、react、angular等框架，这些框架的文件一般都是不会修改的，而每次打包都需要去解析它们，也会影响打包速度，哪怕做拆分，也只是提高了上线后用户访问速度，并不会提高构建速度，所以如果需要提高构建速度，应该使用**动态链接库**的方式，类似于Windows中的dll文件。
...
1. DllPlugin
   ...
   - **manifest.json**  清单文件 说明打包出的vue文件清单，将来怎么用这些打包文件

2. DllReferencePlugin Reference引入
...
3. 删除掉 new CleanWebpackPlugin(), 

   ```
   在webpack.base.js删除new CleanWebpackPlugin() 否则每次打包都会清除vue_dull.js文件
...
5. 安装add-asset-html-webpack-plugin

   `npm i add-asset-html-webpack-plugin -D`

6. 在webpack.base.js中配置插件自动添加script标签到HTML中

   ```js
   注意要在new HtmlWebpackPlugin之后添加
...
3. webpack.base.js修改loader的配置规则
...
output: {
    ...
    path: path.join(__dirname, '..', './dist/'),
    // filename: 'bundle.js',
    filename: '[name].[contenthash:8].bundle.js', //contenthash根据生成的内容生成hash码每次内容修改都会变
    publicPath: '/'
  },
```
...
2. 新建`bin`目录，建立js文件，将打包工具主程序放入其中
...
3. 在`package.json`中配置`bin`脚本
   ...
   ```json
    "bin": {
       "itheima-pack": "./bin/itheima-pack.js"
     },
   //指令找的文件地方
...
4. 通过`npm link`链接到全局包中，供本地测试使用（只需链接一次）

   ```
   C:\Users\何超凡\AppData\Roaming\npm\itheima-pack -> C:\Users\何超凡\AppData\Roaming\npm\node_modules\webpack_theory\bin\itheima-pack.js
   将C:\Users\何超凡\AppData\Roaming\npm\node_modules\webpack_theory\bin\itheima-pack.js链接到全局npm中 在C:\Users\何超凡\AppData\Roaming\npm文件中可以找到itheima-pack这个指令
   ```

   5.运行指令itheima-pack

   ```
   PS C:\Users\何超凡\Desktop\总结\模块化webpack\我的笔记\myCode\4webpack原理\webpack_theory> itheima-pack
   我要开始打包了
   ```
...
这是是一个闭包函数
function (modules) {}1外部函数
function __webpack_require__(moduleId) {}2内部函数   3内部函数引用外部函数变量installedModules
return __webpack_require__（） 4return内部函数
5外部函数调用
(function (modules) {})({})
...
自调函数function (modules) {} 的参数为{"./src/index.js":(function (module, exports, __webpack_require__) {eval("let news = __webpack_require__(/*! ./news.js */ \"./src/news.js\")\r\nconsole.log(news.content)\n\n//# sourceURL=webpack:///./src/index.js?");
})}
键名"./src/index.js"  键值(function (module, exports, __webpack_require__) {eval("let news = __webpack_require__(/*! ./news.js */ \"./src/news.js\")\r\nconsole.log(news.content)\n\n//# sourceURL=webpack:///./src/index.js?");
})


modules[moduleId].call(module.exports, module, module.exports, __webpack_require__);
等价于
(function (module, exports, __webpack_require__) {eval("let news = __webpack_require__(/*! ./news.js */ \"./src/news.js\")\r\nconsole.log(news.content)\n\n//# sourceURL=webpack:///./src/index.js?");
}).call(module.exports, module, module.exports, __webpack_require__) //module.exports, module, module.exports, __webpack_require__的this重新指向键值函数
传入__webpack_require__ 递归调用
```



```js
npm i @babel/parser -S //变成抽象语法树
const parser = require('@babel/parser')
let ast = parser.parse(source)
//变成抽象语法树  查看网站 astexplorer.net
替换抽象语法树中的require

npm i @babel/traverse -S //替换require成__webpack_require__
const traverse = require('@babel/traverse').default
traverse(ast,{}) 替换require成__webpack_require__


npm i @babel/generator -S //将替换好的转为原码
const generator = require('@babel/generator').default
let sourceCode = generator(ast).code

替换let news = require('./news.js') 中./news.js 为./src/index.js给node识别
traverse(ast, {
      CallExpression(p) {
...
// 获取执行itheima-pack指令的目录this.root = process.cwd()
...
// 构建modules对象
    ...
    // this.modules
    let modulePathRelative = './' + path.relative(this.root, modulePath)//绝对路径变为相对路径
...
根据配置文件获取入口，读取入口js代码
depAnalyse分析模块
找到一个个依赖模块，将每一个依赖模块加到modules中，递归构建好this.modules
通过模板引擎进行渲染
输出到对应文字
```
